...
| 工作項 | 必讀段落 |
|:-------|:---------|
| 2b Use Cases 選用圖示 | [[07_diagram_picker]] §2.1（sequence vs state vs activity 試金石）、§3 by role × phase |
| 2c Business Rules 確認 ISO 29148 完整 | [[06_quality_attributes_catalog]] §4 — 每條 rule 需含 ID/source/priority/verification |
| 2d State Model 畫法 | [[07_diagram_picker]] §4.2 起手式、§5 anti-pattern（dead end 必標 `[*]`） |
| 2e Events payload schema 與命名 | [[08_api_design_catalog]] §2.4 event topic naming + §6.3 envelope |
| 2f Integration Inventory 標 API style | [[08_api_design_catalog]] §1（REST/GraphQL/gRPC/event/WebSocket 選哪個）、§5 by role |
| 任何 UC 觸及個人資料 / 敏感資料 | [[11_data_and_stack_catalog]] §1（4 級分類）、§2（PII 三類）、§3.2（GDPR/個資法）— Business Rules 內顯化合規規則 |
