...
## Phase 1.5: 議題 → Catalog 映射（注入龍蝦上下文）

Roundtable 自己**不讀 KB**。但會在 Round 1 dispatch 前，依議題 keyword 把對應 catalog 段落**列入龍蝦的背景文件**，讓龍蝦發言有「該怎麼選」的依據，不憑感覺。

**映射表**（議題 keyword → 注入給龍蝦的 KB 段落）：

| 議題 keyword | 注入給龍蝦的 KB 段落 |
|:-------------|:---------------------|
| KPI / 成功指標 / metric / 量測 | [[06_quality_attributes_catalog]] §1 9 維度、§3 DORA<br>[[09_observability_catalog]] §3 SLI 命名、§3.3 量法 |
| API / contract / endpoint / breaking | [[08_api_design_catalog]] §1 quick picker、§4 versioning、§5 error model |
| schema / migration / ERD / index | [[11_data_and_stack_catalog]] §4 DB 對比、§1-§3 data classification |
| PII / 個資 / GDPR / retention | [[11_data_and_stack_catalog]] §1-§3 合規對應、§5 messaging（若涉及） |
| NFR / SLO / capacity / performance | [[06_quality_attributes_catalog]] §1-§3<br>[[10_resilience_patterns]] §4 RTO/RPO/MTTR |
| 失敗模式 / retry / circuit breaker / timeout | [[10_resilience_patterns]] §1-§2 對比、§3 rollout |
| C4 / diagram / 架構圖 / wireframe | [[07_diagram_picker]] §2-§4 起手式、§5 anti-pattern |
| observability / log / metric / trace / alert | [[09_observability_catalog]] §1 三柱、§5 telemetry hook、§6 burn rate alert |
| auth / SSO / OAuth / session | [[11_data_and_stack_catalog]] §6 auth catalog |
| messaging / queue / event bus / kafka | [[11_data_and_stack_catalog]] §5 messaging |
| rollout / canary / blue-green / feature flag | [[10_resilience_patterns]] §3 rollout 對比 + 選擇樹 |
| stack / framework / 技術選型 | [[11_data_and_stack_catalog]] §7 stack selection |

**映射規則**：
1. Facilitator 在 Phase 1 解析議題時跑這張表，從業主訊息與 active context 抽 keyword
2. 命中 ≥ 1 個 catalog → 把該 catalog 路徑 + § 編號列入 round 1 dispatch prompt 的「背景文件」段
3. 命中 0 個（純探索 / 早期 brainstorm 議題） → 不注入，龍蝦自由發揮
4. 命中 ≥ 3 個 → 只注入「與議題最相關」的 top 2（避免 prompt 膨脹）

**注入後龍蝦的義務**：發言時若引用 catalog 觀點，需明文標 `[ref: 08_api_design_catalog §1]`，方便 MoM cross-refs 提取。

**MoM 必填**：本場注入的 catalog 必須出現在 `MoM.md §🔗 Cross References` 段的「Catalog references used」子欄位，並寫入 `notes.yaml.catalog_refs[]`，供 `indexes/catalog_usage.json` 後台聚合。
...
Catalog references（Phase 1.5 注入，**必讀對應段落**）：
{catalog_refs}
範例：
  - devteam_knowledge_base/08_api_design_catalog.md §1 quick picker、§4 versioning
  - devteam_knowledge_base/06_quality_attributes_catalog.md §1 9 維度

你的任務：用聊天室發言的口吻寫 **150 字以內**，包含：
1. 你的初始立場（一句話）
2. 理由（2-3 點，**引用 catalog 觀點時明文標 [ref: <kb-name> §<section>]**）
...
- 若 catalog 未提供你需要的資訊，明文標 [CATALOG_GAP: <你預期該有的決策樹>]，方便 KB 持續完善
```
